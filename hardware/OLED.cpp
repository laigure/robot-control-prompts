...
#ifndef OLED_SCL_GPIO_Port
#define OLED_SCL_GPIO_Port GPIOB
#endif
#ifndef OLED_SCL_Pin
#define OLED_SCL_Pin GPIO_PIN_8
#endif
#ifndef OLED_SDA_GPIO_Port
...
{
    HAL_GPIO_WritePin(OLED_SCL_GPIO_Port, OLED_SCL_Pin, X ? GPIO_PIN_SET : GPIO_PIN_RESET);
}  
/**
  * 函    数：OLED写SDA高低电平
...
//	//...
//}
void OLED_W_SDA(uint8_t Y)
{
    HAL_GPIO_WritePin(OLED_SDA_GPIO_Port, OLED_SDA_Pin, Y ? GPIO_PIN_SET : GPIO_PIN_RESET);
}   
...
{
	int16_t PointAngle;
	PointAngle = (int16_t)(atan2((double)Y, (double)X) / 3.14 * 180);	//计算指定点的弧度，并转换为角度表示
...
void OLED_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	FraNum = (uint32_t)(Number * PowNum + 0.5);		//将小数乘到整数，同时四舍五入，避免显示误差
